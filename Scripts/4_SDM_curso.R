...
atra_loc <- read.csv("E:\\IIAP\\IIAP_workshop\\Resources_USB\\Data\\Sal_atra\\Salamandra_atra_loc_BiH.csv")
view()
...
BiH_rasters_subset <- exclude(bih_rasters, vif_values)
...
# Set biomod options
system("E:/IIAP/IIAP_workshop/Resources_USB/Software/maxent/maxent.jar")
BiomodOptions <- BIOMOD_ModelingOptions()
atra_loc@coords
# Prepare modeling data
Atra_modeling_data <- BIOMOD_FormatingData(resp.var = rep(1, length(atra_loc$Subspecies)), 
                                           expl.var = BiH_rasters_subset,
                                           resp.xy = atra_loc@coords,
                                           eval.resp.xy = NULL,
                                           PA.nb.rep = 2, 
                                           PA.nb.absences = 1000,
                                           PA.strategy = 'random',
...
Atra_model_out <- BIOMOD_Modeling(Atra_modeling_data,
                                  models = "GLM",
...
Atra_model_projection <- BIOMOD_Projection(modeling.output = Atra_model_out,
                                           new.env = BiH_rasters_subset,
                                           proj.name = "current", 
                                           selected.models = "all",
                                           binary.meth = "TSS",
                                           compress = "xz",
...
write.csv(eval_values_em, "Model_assessment.csv", 
          row.names = FALSE)
library(dplyr)
var_imp_values_em <- Atra_model_out %>% 
...
Atra_model_projection@proj@val
(Atra_model_projection)
...
Atra_model_mean_01 <- Atra_model_mean/1000
plot(Atra_model_mean_01)
library(rasterVis)
levelplot(Atra_model_mean_01)
Atra_model_mean_01[Atra_model_mean_01 < 0.1] <- NA

# Prikaz granice i originalnih tačaka
plot(Atra_model_mean_01)

BiH_boundary <- getData("GADM", country = "BA", level = 0)
plot(BiH_boundary, add = TRUE)
plot(atra_loc, add = TRUE)
mapview(Atra_model_mean_01)
# Spašavanje mape kao kml file kojeg je moguće otvoriti u Google Earthu
KML(Atra_model_mean_01,
    file = "E:/IIAP/Atra_niche.kml", overwrite = TRUE)
...
pal <- colorNumeric(c("#FFFFCC", "#e9a3c9", "#af8dc3"),
                    values(Atra_model_mean_01),
                    na.color = "transparent")

atra_leaflet_map <- leaflet() %>% addTiles() %>%
  addRasterImage(Atra_model_mean_01, colors = pal, opacity = 0.7) %>%
  addLegend(pal = pal, values = values(Atra_model_mean_01),
            title = "Value")
...
library(htmlwidgets)
saveWidget(atra_leaflet_map, file="E:/IIAP/Atra_leaflet_map.html")
